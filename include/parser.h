...
namespace parser
{
	...
	public:
		...
		std::unique_ptr<ast::expr::ExprNode> parsePrimary(const tokens::Token& token, lexer::Lexer& input);
	...
	private:
		...
		std::unique_ptr<ast::expr::ExprNode> parseIdentifier(const tokens::Identifier& ident, lexer::Lexer& input);
		std::unique_ptr<ast::expr::ExprNode> parseNumber(const tokens::Number& number);
		std::unique_ptr<ast::expr::ExprNode> parseBinOpRhs(
			int exprPrec,
		...
		void assertIsCharacter(const tokens::Token& tkn, wchar_t target, const std::string& error);
	};
}
