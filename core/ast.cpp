...
namespace ast
{
	namespace expr
	{
		...
		Binary::Binary(wchar_t op, std::unique_ptr<ExprNode> lhs, std::unique_ptr<ExprNode> rhs)
		...
		Call::Call(const std::wstring& callee, std::vector<std::unique_ptr<ExprNode>> args)
	...
	Function::Function(std::unique_ptr<Prototype> proto, std::unique_ptr<expr::ExprNode> body)
